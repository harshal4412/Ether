...
const THEMES = {
  Architecture: {
    accent: "#575757", font: "'Inter', sans-serif", spacing: "0.02em", weight: "300",
    bgTint: "rgba(240, 240, 240, 0.5)", contrast: "1.05", grain: "0.08",
    description: "The silent dialogue between form and void."
  },
  "Noir Cinema": {
    accent: "#ffffff", font: "'Courier New', monospace", spacing: "-0.01em", weight: "400",
    bgTint: "rgba(0, 0, 0, 0.95)", contrast: "1.4", grain: "0.15",
    description: "Shadows that speak louder than words."
  },
  Cyberpunk: {
    accent: "#00ff41", font: "'Orbitron', sans-serif", spacing: "0.1em", weight: "900",
    bgTint: "rgba(10, 0, 20, 0.9)", contrast: "1.2", grain: "0.03",
    description: "High tech, low life. The digital ghost in the machine."
  },
  General: {
    accent: "#b1976b", font: "'Playfair Display', serif", spacing: "0.2em", weight: "900",
    bgTint: "transparent", contrast: "1.0", grain: "0.03",
...
const AppContent = ({ 
  ...
  return (
    <div className="min-h-screen relative selection:bg-magazine-accent bg-[#fdfdfc] dark:bg-[#0a0a0a] transition-colors duration-500">
      ...
      <TopNav darkMode={darkMode} setDarkMode={setDarkMode} user={user} onAuthAction={handleAuth} />
        ...
        <Route path="/archive" element={
          <>
            ...
            <AnimatePresence mode="wait">
              <motion.main key={selectedCategory} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.8, ease: "anticipate" }} className="max-w-[1800px] mx-auto px-6 md:px-12 pb-32">
                <div className="mb-16 pt-10 border-l border-magazine-accent/30 pl-8">
                  <motion.div key={selectedCategory + "title"} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }}>
                    <h2 className="text-4xl md:text-6xl font-serif mb-4 lowercase tracking-tight">{selectedCategory}</h2>
                  </motion.div>
                  <motion.p key={selectedCategory + "desc"} initial={{ opacity: 0 }} animate={{ opacity: 0.6 }} transition={{ delay: 0.4, duration: 1 }} className="italic font-serif text-lg md:text-xl tracking-wide max-w-2xl">
                ...
                {processedClips.length > 0 ? (
                  <ClipGrid clips={processedClips} selectedIds={selectedIds} onToggleSelection={(id) => setSelectedIds(prev => prev.includes(id) ? prev.filter(i => i !== id) : [...prev, id])} onDeleteClip={deleteClip} onUpdateClip={async (id, updates) => {
...
function App() {
  ...
  useEffect(() => {
    ...
    setCategories(storedCats); setDarkMode(storedDark); setPaperIntensity(storedIntensity); setAccentColor(storedAccent);
  }, []);

  useEffect(() => {
    // SAFETY TIMEOUT: If nothing happens in 2.5 seconds, force the app to show.
  ...
  const addClip = async (newClip) => {
    ...
    const { data, error } = await supabase
      .from('clips')
      .insert([{ ...newClip, user_id: user.id, category: selectedCategory, timestamp: new Date().toISOString() }])
      .select();
    if (error) toast.error("Archive Failed");
    else { setClips([data[0], ...clips]); toast.success('Archived to Cloud'); }
  };
  ...
  const handleMagicLink = async (e) => {
    ...
    if (error) toast.error(error.message);
    else { toast.success("Access link sent to email"); setShowAuthModal(false); }
  };
  ...
  const deleteClip = async (id) => {
    ...
    else { setClips(clips.filter(c => c.id !== id)); setSelectedIds(selectedIds.filter(sid => sid !== id)); toast.error('Entry Removed'); }
  };
  ...
  const clearVault = async () => {
    if(window.confirm("Permanently wipe your cloud anthology?")) {
      ...
      if (!error) { setClips([]); setAccentColor('#b1976b'); setSelectedIds([]); toast.error('Vault Cleared'); }
    }
  };
  ...
  const bulkAddTag = async (tag) => {
    ...
    setClips(updatedClips); setSelectedIds([]); toast.success(`Applied to ${selectedIds.length} folios`);
  };

  const resetFilters = () => { setSearchQuery(''); setFilterType('all'); setActiveTag(null); };
